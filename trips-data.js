(() => {
  ...
  const render = (trip) => {
    body.innerHTML = `<p class="trip-location">${escapeHtml(trip.status)} · ${escapeHtml(trip.location)}</p><h2>${escapeHtml(trip.title)}</h2><p>${escapeHtml(trip.summary)}</p><dl class="trip-facts"><div><dt>Duration</dt><dd>${escapeHtml(trip.duration)}</dd></div><div><dt>Difficulty</dt><dd>${escapeHtml(trip.difficulty)}</dd></div><div><dt>Group size</dt><dd>${escapeHtml(trip.groupSize)}</dd></div></dl><h3>Highlights</h3><ul>${list(trip.highlights)}</ul><h3>Sample itinerary</h3><ul>${list(trip.itinerary)}</ul><h3>Included</h3><ul>${list(trip.included)}</ul><p class="trip-note">${escapeHtml(trip.notes)}</p><a class="btn btn-primary" href="contact.html">${escapeHtml(trip.inquiryLabel)}</a>`;
    dialog.showModal();
  };
